...
RGBLedPattern LED::GeneratePattern(LED_Color color, LED_PatType type, float delay_time, float loop_delay, u8 smooth, float r_shift, float g_shift, float b_shift) {
	...
	switch(type) {
		case LED_PatType::WAVE_DESC:			
			for(int i = 0; i < 32; i++) {
				x = ((float)i/31) * 2*3.14;
				pat.r[i] = ((cos(x + (2*3.14 * modf(r_shift, &dummy))) + 1) / 2) * color.r;
				pat.g[i] = ((cos(x + (2*3.14 * modf(g_shift, &dummy))) + 1) / 2) * color.g;
				pat.b[i] = ((cos(x + (2*3.14 * modf(b_shift, &dummy))) + 1) / 2) * color.b;
			}
			break;
		case LED_PatType::WAVE_ASC:
			for(int i = 0; i < 32; i++) {
				x = ((float)i/31) * 2*3.14;
				pat.r[i] = ((cos(x + (2*3.14 * modf(r_shift, &dummy)) + 3.14) + 1) / 2) * color.r;
				pat.g[i] = ((cos(x + (2*3.14 * modf(g_shift, &dummy)) + 3.14) + 1) / 2) * color.g;
				pat.b[i] = ((cos(x + (2*3.14 * modf(b_shift, &dummy)) + 3.14) + 1) / 2) * color.b;
			}
			break;
